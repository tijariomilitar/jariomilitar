...
Catalog.view.filter = (products, pagination) => {
	...
	if(products.length){
		for (let i = pagination.page * pagination.pageSize; i < products.length && i < (pagination.page + 1) * pagination.pageSize; i++){
			if(!products[i].pack){
				html += "<div class='box b3 container ground margin-top-10 padding-10 pointer' onclick='Catalog.product.controller.show(`"+products[i].id+"`, `"+products[i].category_id+"`)'>";
			} else {
				html += "<div class='box b3 container ground margin-top-10 padding-10 pointer' onclick='Catalog.package.controller.show(`"+products[i].id+"`, `"+products[i].category_id+"`)'>";
			};
			if(products[i].image){
			...
			if(products[i].price){ html += "<div class='box b1 em15 center italic bold' style='color:#467846;'>$"+products[i].price.toFixed(2)+"</div>" }
			html += "</div>";
		};
